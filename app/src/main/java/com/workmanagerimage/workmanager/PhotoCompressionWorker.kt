package com.workmanagerimage.workmanager
...
class PhotoCompressionWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        return withContext(Dispatchers.IO){
            var stringUri = workerParameters.inputData.getString(KEY_CONTENT_URI)

            val keyCompressionThresholdInBytes = workerParameters.inputData.getLong(
            ...
            }while (outputBytes.size>keyCompressionThresholdInBytes && quality>5)

            val file = File(appContext.cacheDir,"${workerParameters.id}.jpg")
